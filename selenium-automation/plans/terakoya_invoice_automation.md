...
3. フィールドに入力:
   - 日付: レッスン日を選択
   - カテゴリー: "専属レッスン"を選択
   - 受講生: ドロップダウンから選択（スクロール対応）
   - 専属レッスン: 受講生選択後に表示されるレッスン一覧から選択
   - 時間: 60
   - 単価: 2300
4. 入力内容を確認
5. モーダル下部の赤い「追加」ボタンをクリック
6. モーダルが閉じるまで待機
7. 成功/失敗を返却
...
12. 送信実行
    ├─ TerakoyaClient.submit_invoice()
    ├─ スクリーンショット: invoice_submitted.png
    └─ ログ出力: "送信完了"
