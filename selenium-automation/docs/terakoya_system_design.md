...
[開始]
  ...
  │      ├─ 各レッスンカードのテキストを取得
  │      ├─ **AI抽出（主要）** or 正規表現（フォールバック）で情報を抽出
  │      │   ├─ **AI抽出**: Claude Sonnet 4.5（Vertex AI経由）
  │      │   │   - バッチ処理: デフォルト10件/API呼び出し
  │      │   │   - 環境変数: TERAKOYA_USE_AI_EXTRACTION=true
  │      │   │   - 高精度な人名抽出（「最終レッ」「キャンセ」等を除外）
  ...
  │      ├─ `/claim` URLへ直接遷移
  │      └─ 対象月を選択（ドロップダウンから YYYY/MM 形式で選択）
  │
  ...
  │        │   ├─ モーダルをJavaScriptでクリックして開く
  │        │   ├─ フォーム入力:
  │        │   │   - 日付: React DatePicker（JavaScript経由、YYYY年MM月DD日形式）
  │        │   │   - カテゴリー: 数値ID（1-26）にマッピング
  │        │   │   - 受講生: 標準SELECT or カスタムReactドロップダウン
  │        │   │   - レッスン: API非同期読み込み待機（最大12秒）
  │        │   │   - 時間・単価: JavaScript経由で値設定
  │        │   ├─ dry_runモード: スクリーンショット保存、モーダル開いたまま終了
  │        │   ├─ 通常モード: 追加ボタンをクリック
  │        │   ├─ 成功/失敗を記録
  │        │   └─ スクリーンショット保存
  │        └─ エラー時はスキップして継続
  ...
  └─ 11. ブラウザクローズ
         ├─ ログ出力: "請求項目の追加が完了しました"
         └─ [終了] → **請求書の送信は自動化されていません（Web画面から手動で送信）**
```
...
#### 6.5.1 React コンポーネント対応

Terakoyaサイトは React ベースのモダンフロントエンドを使用しており、以下の特殊な処理が必要です:

**React DatePicker (日付入力)**:
- 標準的な Selenium の `send_keys()` では値が反映されない
- JavaScript経由で値を設定し、React の change イベントを手動でディスパッチ
- 日付フォーマット: `YYYY年MM月DD日` (例: `2025年10月01日`)
- 実装: `set_value_javascript()` メソッドを使用
- フォールバック: JavaScript失敗時は手動入力 + TABキー

**カスタムReactドロップダウン (受講生選択)**:
- 標準HTMLの`<select>`要素ではなく、styled-componentsによるカスタムコンポーネント
- 動的クラス名（`sc-eYHxxX`, `sc-eVrRMb`等）を使用
- 検索機能付き: プレースホルダ='検索' の input 要素
- 選択フロー:
  1. ドロップダウン表示要素をクリック
  2. 検索inputに受講生名を入力
  3. ENTERキーで確定
  4. 一致するオプションを選択
  5. change イベントをディスパッチ
- フォールバック: 標準SELECT要素の存在を先にチェック

**JavaScript Click 要件**:
- ログインボタン (div要素) のクリックにはJavaScript必須
- 「請求項目の追加」ボタンも同様にJavaScript clickが必要
- 理由: クライアントサイドルーティングとイベントリスナーの仕様

**API非同期読み込み (レッスン選択)**:
- 受講生選択後、レッスンドロップダウンのオプションがAPI経由で非同期読み込み
- 最大12秒の明示的待機が必要
- 待機条件: `len(Select(element).options) > 1` (プレースホルダ以外のオプションが存在)

#### 6.5.2 デバッグ機能

実装には以下のデバッグ支援機能が含まれます:

**HTMLスナップショット保存**:
- `output/terakoya_debug/` ディレクトリにページソースを保存
- トリガーポイント:
  - `modal_student_dropdown_missing`: 受講生ドロップダウンが見つからない
  - `modal_student_dropdown_input_missing`: 検索inputが見つからない
  - `modal_student_dropdown_no_options`: オプションが読み込まれない
  - `modal_student_dropdown_mismatch`: 選択値が一致しない
- ファイル形式: `modal_{tag}_{timestamp}.html`
- 用途: Reactコンポーネントの問題診断、セレクタ調整

**dry-runモード**:
- 環境変数: `--dry-run` フラグで有効化
- 動作:
  - 請求項目フォームに入力まで実行
  - 「追加」ボタンは **クリックしない**
  - モーダルを開いたまま残す（手動確認可能）
  - スクリーンショット保存: `dry_run_form_filled_{timestamp}.png`
- 用途: フォーム入力の検証、実際に送信せずに確認
